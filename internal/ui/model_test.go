...
func TestModelRendersFileSystemsTab(t *testing.T) {
	...
	m.fileSystems = []eosgrpc.FileSystemRecord{
		{
			ID:            3,
			Host:          "host",
			Port:          1095,
...
func TestFileSystemsListScrollsToSelectedEntry(t *testing.T) {
	...
	for i := 0; i < 30; i++ {
		m.fileSystems = append(m.fileSystems, eosgrpc.FileSystemRecord{
			ID:         uint64(i),
...
func TestNarrowResizeStillFitsWindowHeight(t *testing.T) {
	...
	for i := 0; i < 40; i++ {
		m.directory.Entries = append(m.directory.Entries, eosgrpc.Entry{
			Name: "entry-" + padIndex(i) + "-with-a-long-name",
			Path: "/entry-" + padIndex(i),
			Kind: eosgrpc.EntryKindFile,
			UID:  1000,
			GID:  1000,
		})
	}
